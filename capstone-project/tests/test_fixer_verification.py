...
def test_medium_page_has_reduction_from_fix():
    ...
    assert any(s.get('reduction', 0) > 0 for s in suggestions), 'No suggestion produced a positive reduction'
    ...
    for s in suggestions:
        ...
        if not any(i.get('id') == orig_id for i in post.get('issues', [])):
            ok = True
            break
    assert ok, 'No suggestion resulted in a post-scan that removed the original issue id'
