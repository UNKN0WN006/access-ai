...
def test_medium_alt_missing_fixed():
    ...
    assert any(
        img.get("alt") == "Describe image" for img in soup.find_all("img")
    ), 'No alt="Describe image" found after patch'
    ...
    assert not any(
        i.get("id") == "alt_missing" for i in post.get("issues", [])
    ), "alt_missing still present after patch"
...
def test_medium_heading_added():
    ...
    assert not any(
        i.get("id") == "heading_structure" for i in post.get("issues", [])
    ), "heading_structure still present after patch"
...
def test_bad_label_missing_id_fixed():
    ...
    assert not any(
        i.get("id") == "label_missing_id" for i in post.get("issues", [])
    ), "label_missing_id still present after patch"
